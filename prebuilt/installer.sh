...
on_install_failed() {
  ...
  cp -f $INTERNAL/addon-config.prop $TMP/bitgapps/addon-config.prop 2>/dev/null;
  cp -f $INTERNAL/cts-config.prop $TMP/bitgapps/cts-config.prop 2>/dev/null;
  cp -f $INTERNAL/setup-config.prop $TMP/bitgapps/setup-config.prop 2>/dev/null;
  echo "$ANDROID_ROOT" >> $TMP/bitgapps/mount.log 2>/dev/null;
...
get_setup_config_path() {
  for s in $(find /sdcard* -iname "setup-config.prop" 2>/dev/null;); do
    if [ -f "$s" ]; then
      SETUP_CONFIG_DEST="$s"
    fi;
  done
...
get_cts_config_path() {
  for c in $(find /sdcard* -iname "cts-config.prop" 2>/dev/null;); do
    if [ -f "$c" ]; then
      CTS_CONFIG_DEST="$c"
    fi;
  done
...
get_addon_config_path() {
  for a in $(find /sdcard* -iname "addon-config.prop" 2>/dev/null;); do
    if [ -f "$a" ]; then
      ADDON_CONFIG_DEST="$a"
    fi;
  done
...
ext_pathmap() {
  if [ "$android_sdk" == "$supported_sdk_v30" ]; then
    if [ "$dynamic_partitions" == "true" ]; then
      ...
      test -d $SYSTEM_ETC_CONFIG && chmod 0755 $SYSTEM_ETC_CONFIG || return 0;
      test -d $SYSTEM_ETC_DEFAULT && chmod 0755 $SYSTEM_ETC_DEFAULT || return 0;
      test -d $SYSTEM_ETC_PREF && chmod 0755 $SYSTEM_ETC_PREF || return 0;
      test -d $SYSTEM_LIB && mkdir chmod 0755 $SYSTEM_LIB || return 0;
      test -d $SYSTEM_LIB64 && chmod 0755 $SYSTEM_LIB64 || return 0;
      test -d $SYSTEM_ETC_CONFIG && chcon -h u:object_r:system_file:s0 $SYSTEM_ETC_CONFIG || return 0;
      test -d $SYSTEM_ETC_DEFAULT && chcon -h u:object_r:system_file:s0 $SYSTEM_ETC_DEFAULT || return 0;
      test -d $SYSTEM_ETC_PREF && chcon -h u:object_r:system_file:s0 $SYSTEM_ETC_PREF || return 0;
      test -d $SYSTEM_LIB && chcon -h u:object_r:system_file:s0 $SYSTEM_LIB || return 0;
      test -d $SYSTEM_LIB64 && chcon -h u:object_r:system_file:s0 $SYSTEM_LIB64 || return 0;
    fi;
  fi;
}
